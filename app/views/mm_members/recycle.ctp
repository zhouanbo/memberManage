...
$(document).ready(function(){
	$('#control2').show();
	$('#c2').html('<a href="#" id="filter" class="button">筛选</a>');
...
echo '<fieldset><legend>回收站</legend>';
...
if($group=='admin' || $group=='superAdmin')
	echo '<td>'.$form->input('group',array('label'=>'用户组：','empty'=>'无','options'=>array('clerk'=>'干事','minister'=>'部长','admin'=>'管理员'))).'</td>';
...
<td><?php echo $html->link('恢复',array('controller'=>'MmMembers','action'=>'recover',$member['MmMember']['id']),null,"确认恢复吗？"); ?></td>
<td><?php echo $html->link('彻底删除',array('controller'=>'MmMembers','action'=>'clear',$member['MmMember']['id']),null,"记录将无法恢复，请三思！"); ?></td>
<?php } ?>
</tr>
<?php endforeach;?>
</table>
